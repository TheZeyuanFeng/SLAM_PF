...
def run_slam(src_dir, log_dir, idx, split):
    ...
    while slam.lidar[t0_lidar]['t'] < t0: t0_lidar += 1
    ## Synchronize lidar and joint

    xyth = slam.lidar[t0_lidar]['xyth']
    xyth[2] = slam.lidar[t0_lidar]['rpy'][2]
    logging.debug('> Initializing 1 particle at: {}'.format(xyth))
    # initialize the occupancy grid using one particle and calling the observation_step
    # function
    #### TODO: XXXXXXXXXXX
    slam.init_particles(n=1,p=xyth.reshape((3,1)),w=np.array([1])) #Run one step with only one particle to initialize map
    ...
    slam.init_particles(n=10,p=None,w=None)
    ...
    # T=100
    particle_traj=np.zeros((2,T-1))
    for t in tqdm.tqdm(range(t0_lidar+1,T)):
        slam.dynamics_step(t)
        pm=slam.observation_step(t)
        particle_traj[:,t-1]=pm[0:2]

    ## plot ##
    
    plt.figure(2); plt.clf();
    plt.title('Trajectory using onboard odometry')
    plt.plot(xyth[0:T,0], xyth[0:T,1],'r')
    plt.plot(particle_traj[0,:],particle_traj[1,:],'b')
    plt.figure(2)
    
    plt.imshow(slam.map.cells, cmap='gray_r')

    # plt.scatter(particle_traj[0,:],particle_traj[1,:],color='b')
    odom_map=slam.map.create_traj_map(xyth[0:T,0],xyth[0:T,1])
    traj_map=slam.map.create_traj_map(particle_traj[0,:],particle_traj[1,:])
    plt.imshow(odom_map, cmap='Reds',alpha=0.2)
    plt.imshow(traj_map, cmap='Blues',alpha=0.2)
    # plt.plot(xyth[0:T,0], xyth[0:T,1],'r')
    # plt.plot(particle_traj[0,:],particle_traj[1,:],'b')
    plt.show()
